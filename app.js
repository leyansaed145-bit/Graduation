function showThankYou(name) {
  ...
  if (!toast || !msg || !closeBtn) return;

  msg.textContent = `Thank you ${name} — your gift has been reserved 💛`;

  const hide = () => toast.classList.remove("show");

  closeBtn.onclick = hide;
  toast.onclick = (e) => { if (e.target === toast) hide(); };

  toast.classList.add("show");

  // auto close after 3.5s
  clearTimeout(window.__toastTimer);
  window.__toastTimer = setTimeout(hide, 3500);
}
...
async function fetchGifts() {
  ...
  if (!res.ok) {
    statusEl.textContent = "Could not load gifts.";
    return [];
  }
...
const body = {
  ...
  if (res.ok) {
    ...
    if (!updated || updated.length === 0) alert("Someone has already reserved it 😅");
    else showThankYou(name.trim());
  } else {
